...
describe('Atom Feed Item Adding', () => {
  it('Should add the new item', () => {
    ...
    const atomFeed = new AtomFeed(metadata, []);
    const beforeCount = atomFeed.getItems().length;
    atomFeed.addItem(item);
    const afterCount = atomFeed.getItems().length;
