...
describe('JSON Feed Item Adding', () => {
  it('Should add the new item', () => {
    ...
    const jsonFeed = new JsonFeed(metadata, []);
    const beforeCount = jsonFeed.getItems().length;
    jsonFeed.addItem(item);
    const afterCount = jsonFeed.getItems().length;
