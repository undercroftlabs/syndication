...
describe('JSON Feed Serialization', () => {
  it('Should serialize without an author', () => {
    ...
    const expectedFeed = fs.readFileSync(
      path.join(__dirname, '__snapshots__', 'expected-no-author.json'),
      'utf8',
    );
  ...
  it('Should serialize without a html content', () => {
    const metadata = {
      title: 'Undercroft Labs',
      description: 'Tiny tools. Big ideas.',
      id: 'https://undercroftlabs.com/feed',
    ...
    const renderedFeed = jsonFeed.render();
    const expectedFeed = fs.readFileSync(
      path.join(__dirname, '__snapshots__', 'expected-no-html-content.json'),
      'utf8',
    );
